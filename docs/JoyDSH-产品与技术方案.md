...
1. 用手柄完成工作空间切换、任务恢复、执行清单查看、方案审阅、审批、成果检查和常用命令。
...
- 切换前台和后台任务。
- 集中处理待回应事项；审批、问题和方案审阅分别使用符合其语义的交互界面。
- 查看完成、失败和断连通知。
- 暂停或停止智能体。
- 打开全局搜索、运行时状态和设置。
...
| 主要操作 / 删除文字 | 西侧按键 | 输入框聚焦时删除文字，非编辑时执行当前项目的高频动作 |
| 更多操作 | 北侧按键 | 打开上下文操作菜单 |
| 精确移动 | 十字键 | 在相邻焦点之间移动 |
| 快速移动 | 左摇杆 | 连续移动焦点或列表滚动 |
| 内容滚动 | 右摇杆 | 阅读日志、差异和长文档 |
| 上一个或下一个区域 | 左、右肩键 | 切换一级焦点区域 |
| 上一个或下一个内容页 | 左、右扳机键 | 切换执行清单、变更、成果等页面 |
| 命令中心 | 菜单键 | 打开或关闭命令中心 |
...
- 长文本默认使用实体键盘。
- 支持外部听写桥：通过系统底层按键模拟（如 macOS `Right Command` / `0x36`）联动 Spokenly、Superwhisper 或系统原生听写。手柄默认使用可配置的 R3；按下与松开事件交由听写工具解释，JoyDSH 保留当前有效输入框并在没有输入目标时回退到前台任务输入框。
...
- 后台任务不会弹出阻断窗口；待回应事项统一进入命令中心。
- 切换前台任务不暂停原任务，除非并发策略要求让出执行名额。
- 任务状态至少包含：草稿、排队、执行中、等待输入、等待审批、已暂停、已完成、失败。
- 前台任务执行中提交的输入默认排队；用户可显式改为纠偏输入，并可撤销尚未执行的排队输入。
- 子任务在父任务下展示父子关系、状态、摘要和历史，并允许继续说明或中断；JoyDSH 不提供手动创建、提供方配置或复杂编排界面。
- 方案模式与执行清单分别投影；方案审阅作为待回应事项处理，不能用执行清单的完成状态代替审阅决定。
- 持续目标展示目标内容、当前轮次和生命周期状态，并允许暂停、恢复和结束；首版不提供复杂预算或自动化管理界面。
- 后台作业展示状态、输出和停止操作；Workflow 展示阶段、日志和关联子任务并允许取消，但不提供脚本编辑或可视化编排。
...
- 提供会话、智能体、执行清单、方案模式、持续目标、审批、成果和运行状态能力。
- 默认只监听本机回环地址。
...
当前里程碑不挂载 MCP、Schedule、LSP、Hooks、ACP、E2B 或插件管理界面。JoyDSH 先建立并测试专属 profile 与 patch 边界，后续能力分别评估凭据、信任、生命周期和手柄交互后再立项。

一元命令通过 DSH 的类型化远程调用能力完成，例如创建任务、发送输入、停止任务和回应审批。适配层同时提供 DSH 命令的发现、执行和直接结果协议；命令中心只精选高频命令，任务输入框通过 `/` 入口发现完整命令目录。会话事件、流式模型输出和增量状态通过独立事件流传输，不伪装成普通远程方法，也不依赖高频轮询。
...
工作空间：添加、移除、选择、最近使用
任务会话：创建、恢复、列出、重命名、按标题或元数据搜索、发送、纠偏、队列编辑、暂停、停止、派生、完整历史回放
事件：订阅、续接、回放、缺口修复
待回应：聚合、提问回答、方案审阅、审批允许与拒绝
智能体：执行清单、方案模式、持续目标、子任务监督
后台执行：后台作业状态、输出与停止，Workflow 阶段、日志与取消
命令：发现、执行、直接结果
模型：智能体预设、会话模型目录、推理强度
上下文：占用、压缩记录、手动压缩
成果：文件、差异、执行清单、终端结果和生成文件
权限：读取和切换标准权限、完全访问
```

所有 DSH 特定类型必须在适配层内转换成 JoyDSH 领域类型。每个支持的 DSH 版本都运行同一套契约测试；不满足契约时拒绝自动升级。

会话引用、ZIP 导出、复杂排序和 DSH 原生工作空间管理不属于首批适配契约，在核心恢复、重命名、派生和搜索稳定后再评估。

### 8.6 DSH 能力收录优先级

JoyDSH 按任务闭环而不是 DSH Web 功能数量安排集成：

**必须补齐**：

- 结构化问题、方案审阅及其回应协议，避免任务会话停在不可处理的等待状态。
- 完整历史分页与恢复，避免长任务会话丢失事实。
- 排队输入、纠偏输入和撤销尚未执行输入。
- 待回应聚合，并保持审批、问题和方案审阅的独立语义。

**核心增强**：

- 子任务关系、状态、历史、继续说明和中断。
- 执行清单、方案模式和持续目标生命周期。
- 后台作业与 Workflow 的状态、输出、日志、停止和取消。
- DSH 命令发现、执行与直接结果。
- 任务会话重命名、派生和按标题或元数据搜索。
- 使用 DSH presentation metadata 展示可折叠工具卡和调用层级；未知类型使用通用回退。

**完成核心增强后接入**：

- 在创建空白任务时选择 `standard` 或 `code` 智能体预设；其他预设和预设编辑器后置。
- 通过 `session.models` 展示会话可用模型和推理强度，不维护硬编码模型目录。
- 显示简洁的上下文占用、压缩记录和手动压缩入口，不建设详细 token 或成本分析台。

**明确后置**：

- 会话引用、ZIP 导出、消息反馈、详细会话统计、Skill picker 和自定义智能体预设编辑器。
- MCP、Schedule、LSP、Hooks、ACP、E2B、插件管理和复杂多智能体或 Workflow 编排。
...
- 支持一个前台任务和两个后台执行任务。
- 待回应来源和类型始终可见；审批还必须显示命令、影响范围和权限模式。
